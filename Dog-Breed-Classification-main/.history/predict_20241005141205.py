...
def load_confusion_matrix():
    try:
        ...
        # โหลดภาพทั้งสองและปรับขนาด
        images = [Image.open(path).resize((300, 200)) for path in paths]

        # รวมภาพทั้งสองเข้าด้วยกันในแนวนอน
        combined_image = ImageOps.concat(images)

        # แปลงภาพที่รวมแล้วเป็น ImageTk
        combined_image_tk = ImageTk.PhotoImage(combined_image)

        # แสดงภาพที่รวมกันในป้าย
        label_image.config(image=combined_image_tk)

        # เก็บ reference ของภาพเพื่อไม่ให้ถูก garbage collected
        label_image.image = combined_image_tk
